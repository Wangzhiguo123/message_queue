...
import { getList,AddApp,Deletefn,updateApp,tenantList,topicList,InsertApp } from '@/api/theme'
export default {
  name: 'Home',
  components: {
    Pages
  ...
  data() {
    return {
      ...
      data: {
        ...
        select: [
          { title: 'namespaceName', label: '主题', type: 'select2' },
          { title: 'topicDesc', label: '描述', type: 'text' },
  ...
  methods: {
    ...
    async d_action(type, newdata, index) {
      if (type == 'Add') {
        newdata.topicInfoId=newdata.namespaceName[2]
        delete newdata.namespaceName
        // console.log(newdata)
        let formData = new FormData()
        handleParam(newdata,formData)
        // console.log(formData)
        // return
        const datas = await InsertApp(formData)
        console.log(datas)
        if (datas) {
    ...
    async Tabular(newdata) {
      ...
      try {
        ...
        if(ListData.data){
          // console.log('ok')
          this.data.total = ListData.data.data.total?ListData.data.data.total:0
          // this.list = ListData.data.data.list?ListData.data.data.list:null
